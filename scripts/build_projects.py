...
def load_projects(root):
    ...
    for record in records:
        ...
        for field in ("status", "engine", "role", "cover", "steam_url", "itch_url", "bilibili_url", "pdf_url", "play_url", "download_url", "source_url"):
...
def project_card(record, root, eager=False):
    ...
    actions = []
    for field, label in [("steam_url", "在 Steam 查看"), ("itch_url", "在 itch.io 查看"), ("bilibili_url", "在 B 站观看"), ("pdf_url", "查看作品集（PDF）"), ("play_url", "开始游玩"), ("download_url", "下载游戏"), ("source_url", "查看源码")]:
        if record.get(field):
